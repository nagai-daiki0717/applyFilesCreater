...
public class KotsuhiExcelService {
    ...
    private java.util.List<String> applyCommuteApplicationToWorkbook(
        ...
        XSSFSheet wsMain = wb.getSheet(ConstValues.KOTSUHI_SHEET_NAME);
        if (wsMain == null) {
        ...
        if (submitDate != null) {
            setCell(wsMain, ConstValues.SUBMIT_DATE_CELL, submitDate.format(
                    DateTimeFormatter.ofPattern(
                            ConstValues.JP_YYYYMMDD_PATTERN)));
        }
        setCell(wsMain, ConstValues.CREATE_MONTH_CELL, submitDate.format(
                DateTimeFormatter.ofPattern(ConstValues.CREATE_MONTH_PATTERN)));
        setCell(wsMain, ConstValues.APPLICANT_NAME_CELL, applicantName);
        setCell(wsMain, ConstValues.APPLICANT_NO_CELL, applicantNo);
        setCell(wsMain, ConstValues.APPLICANT_PROJECT_NAME_CELL, applicantProjectName);
        ...
        for (int i = 0; i < carfareDetails.size(); i++) {
            ...
            String tripType = detail.path("carfare_one_way_or_round_trip")
                    .asText("");
    ...
    public String buildOutputFilename(JsonNode payload) {
        LocalDate submitDate = parseDate(text(payload, "submit_date"));
        String yyyymm = String.format("%d年度%02d月", submitDate.getYear(),
                submitDate.getMonthValue());

        String applicant = normalizeApplicantName(text(payload,
                "applicant_name"));
